...
import com.softwinner.splitscreen.SplitCtrlWindowView;
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode {
    ...
    SplitCtrlWindowView mSplitCtrlWindow;
    int mAppDispWidth;
    ...
    private ImageView mStatusFM;
    private TextView mStatusGps;
    private ImageView mStatusMute;
    private ImageView mStatusWifi;
    private ImageView mStatusBt;
    ...
    @Override
    public void setSplitButtonVisibility(boolean isVisible) {
        if (SPLITSCREEN) {
            mSplitCtrlWindow.setButtonVisibility(isVisible);
        } else {
            Log.e(TAG, "hcl:Split screen API has been call but system not under split screen mode.");
        }
    }
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        if (SPLITSCREEN) {
            mStatusBarWindow = (StatusBarWindowView) View.inflate(context,
                    R.layout.super_status_bar_splitscreen, null);
        } else {
            mStatusBarWindow = (StatusBarWindowView) View.inflate(context,
        ...
        if (SPLITSCREEN) {
            mSplitCtrlWindow = (SplitCtrlWindowView) View.inflate(context,
                R.layout.split_ctrl_button, null);
            mSplitCtrlWindow.setImgViewId(R.id.split_ctrl_img);
            mSplitCtrlWindow.setButtonImgId(R.drawable.control_bar_hide_night_normal, R.drawable.control_bar_show_night_normal);
        }

        if (SPLITSCREEN) {
            mStatusBarView = (PhoneStatusBarView) mStatusBarWindow.findViewById(R.id.status_bar_splitscreen);
        } else {
        ...
        try {
            ...
            if (showNav) {
                if (SPLITSCREEN) {
                    mNavigationBarView =
                            (NavigationBarView) View.inflate(context, R.layout.navigation_bar_splitscreen, null);
                } else {
                    mNavigationBarView =
        ...
        if (SPLITSCREEN) {
            mSystemIconArea = (LinearLayout) mNavigationBarView.findViewById(R.id.system_icon_area);
            mStatusIcons = (LinearLayout)mNavigationBarView.findViewById(R.id.statusIcons);
        } else {
        ...
        if (SPLITSCREEN) {
            mHasSettingsPanel = res.getBoolean(R.bool.config_hasSettingsPanel_splitscreen);
            mHasFlipSettings = res.getBoolean(R.bool.config_hasFlipSettingsPanel_splitscreen);
        } else {
        ...
        if (SPLITSCREEN) {
            mBatteryController.addLabelView((TextView)mNavigationBarView.findViewById(R.id.battery_text));
        } else {
        ...
        if (SPLITSCREEN) {
            signalCluster = (SignalClusterView)mNavigationBarView.findViewById(R.id.signal_cluster);
        } else {
        ...
        if (mHasSettingsPanel) {
            ...
            if (mSettingsContainer != null) {
                ...
                if (!mNotificationPanelIsFullScreenWidth) {
                    if (SPLITSCREEN) {
                        //Remove STATUS_BAR_DISABLE_SYSTEM_INFO for [split screen].
        ...
        mStatusGps = (TextView)mStatusBarWindow.findViewById(R.id.status_gps);
        mStatusFM = (ImageView)mStatusBarWindow.findViewById(R.id.status_fm);
        mStatusMute = (ImageView)mStatusBarWindow.findViewById(R.id.status_mute);
        mStatusWifi = (ImageView)mStatusBarWindow.findViewById(R.id.status_wifi);
        mStatusBt = (ImageView)mStatusBarWindow.findViewById(R.id.status_bt);
        ...
        filter.addAction("android.fm.action.set_fmtx_state");
    ...
    public void addIcon(String slot, int index, int viewIndex, StatusBarIcon icon) {
        if (SPEW) Log.e(TAG, "addIcon slot=" + slot + " index=" + index + " viewIndex=" + viewIndex
                + " icon=" + icon);
    ...
    public void addNotification(IBinder key, StatusBarNotification notification) {
        ...
        //split screen dont need notification
    ...
    public void disable(int state) {
        ...
        flagdbg.append(">");
        Log.i(TAG, "½ûÓÃ£º"+state+"flagdbg:"+flagdbg.toString());//expand icons alerts ticker system_info back home recent clock search 
        ...
        if ((diff & StatusBarManager.DISABLE_SYSTEM_INFO) != 0) {
            ...
            if ((state & StatusBarManager.DISABLE_SYSTEM_INFO) != 0) {
                ...
                Log.i(TAG, "mSystemIconArea£º"+mSystemIconArea);
...
//                        event.getActionMasked(), (int) event.getX(), (int) event.getY(), mDisabled);
    ...
    public void setInteracting(int barWindow, boolean interacting) {
        ...
        if (SPLITSCREEN) {
            mHandler.postDelayed(mUpdateTile,0);
        }
        checkBarModes();
    }
    ...
    private final Runnable mUpdateTile = new Runnable() {
        @Override
        public void run() {
            mQS.updateRightWindowAppTiles();
        }};
    ...
    private class MyTicker extends Ticker {
        ...
        public void tickerStarting() {
            mTicking = true;
            //modify by hechuanlong.we dont need ticker!
            if (!SPLITSCREEN) {
    ...
    private void addStatusBarWindow() {
        ...
        if (SPLITSCREEN) {
            mSplitCtrlWindow.initButton();
        }
    }

    public void updateSplitCtrlwin() {
        mSplitCtrlWindow.updateButtonView();
    }
            ...
            if (Intent.ACTION_CLOSE_SYSTEM_DIALOGS.equals(action)) {
               ...
               clearAll();
                ...
                if(mAudioManager.getRingerMode() == 0) {
                    mStatusMute.setSelected(true);
                }else{
                	 mStatusMute.setSelected(false);
                }
            }
            else if(BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
            	 int state = intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, BluetoothAdapter.ERROR);
            	   Log.d("hy","state:"+state);
                if (state == BluetoothAdapter.STATE_ON) {
                	mStatusBt.setSelected(true);
				}else{
					mStatusBt.setSelected(false);
				}
            }
            
            else if("android.fm.action.set_fmtx_state".equals(action)) {
            	   boolean state = intent.getExtras().getBoolean("fmtx_state_value");
           	   Log.d("hy","state:"+state);
               if (state) {
               	mStatusFM.setSelected(true);
				}else{
				mStatusFM.setSelected(false);
				}
           }
        }
    };
...
//            Log.d(TAG, "size=" + newIconSize + " padding=" + newIconHPadding);
    ...
    public void dispatchDemoCommand(String command, Bundle args) {
        ...
        Log.i("hy", "dispatchDemoCommand:"+modeChange+"command:"+command);
