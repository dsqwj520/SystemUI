...
class QuickSettings {
    ...
    private static final boolean SPLITSCREEN = SystemProperties.getBoolean("persist.sys.splitscreen", false);
    ...
    private String[] mSettingAppList = null;
    ...
    public QuickSettings(Context context, QuickSettingsContainerView container) {
        ...
        if (SPLITSCREEN) {
            ...
            String str = Settings.System.getString(resolver, Settings.System.RIGHT_MEMU_APP_LIST);
            mSettingAppList = str.split(Settings.System.SHORTCUT_PATH_SEPARATOR);
            if(mSettingAppList == null){
    ...
    private void setupQuickSettings() {
        ...
        if (SPLITSCREEN) {
            addRightWindowAppTiles(mContainerView, inflater);
        }
    ...
    private void addRightWindowAppTiles (ViewGroup parent, LayoutInflater inflater) {
        ...
        for (ResolveInfo appInfo : list) {
            if (mSettingAppList != null && mSettingAppList[0].length() > 2) {
                boolean isExist = false;
                for (String name : mSettingAppList) {
                    if (name.equals(appInfo.activityInfo.packageName)) {
                        isExist = true;
                        break;
                    }
                }
            ...
            userTile.setContent(R.layout.quick_settings_tile_app, inflater);
            TextView appNameView = (TextView) userTile.findViewById(R.id.app_textview);
            ImageView iconView = (ImageView) userTile.findViewById(R.id.app_imageview);
            String lable = appInfo.activityInfo.loadLabel(pm).toString();
            appNameView.setText(lable);
            iconView.setImageDrawable(appInfo.activityInfo.loadIcon(pm));
            final String pgn = appInfo.activityInfo.packageName;
            final String cls = appInfo.activityInfo.name;
            userTile.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    ComponentName componet = new ComponentName(pgn, cls);
                    Intent intent = new Intent();
                    intent.setComponent(componet);
                    //intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    intent.addFlags(Intent.FLAG_ACTIVITY_RUN_IN_RIGHT_WINDOW);
                    mContext.startActivityAsUser(intent, new UserHandle(UserHandle.USER_CURRENT));
                    collapsePanels();
                }
            });
            parent.addView(userTile);
            AppInfo tileInfo = new AppInfo(userTile, lable, pgn);
            mAppList.add(tileInfo);
        }
    }

    public void addRightWindowAppTiles () {
        LayoutInflater inflater = LayoutInflater.from(mContext);
        addRightWindowAppTiles(mContainerView, inflater);
    }

    class AppInfo {
        QuickSettingsTileView appTile;
        String appLable;
        String packageName;
        AppInfo (QuickSettingsTileView v, String lable, String pgn) {
            appTile = v;
            appLable = lable;
            packageName = pgn;
        }
    ...
    public void updateRightWindowAppTiles () {
        ActivityManager activityManager = (ActivityManager) mContext.getSystemService(Context.ACTIVITY_SERVICE);
        List<ActivityManager.RecentTaskInfo> list = activityManager.getRecentTasks(10, 0);
        //ComponentName componentInfo = taskInfo.get(0).topActivity;
        //Log.d(TAG, "hcl componentInfo.getPackageName=" + componentInfo.getPackageName());
        int size = mAppList.size();
        //step1: get left and right stackId
        int rightStackId = -1;
        int leftStackId = -1;
        try {
            rightStackId = ActivityManagerNative.getDefault().getLeftRightStack(false);
            leftStackId = ActivityManagerNative.getDefault().getLeftRightStack(true);
        } catch (RemoteException e) {
            Log.e(TAG, "get stackId faile", e);
        }
        //step2: get the top running app in left and right stack
        ComponentName rightComponentInfo = null;
        ComponentName leftComponentInfo = null;
        try {
             ActivityManager.RecentTaskInfo rightTask =
                ActivityManagerNative.getDefault().getTopTask(rightStackId);
             ActivityManager.RecentTaskInfo leftTask =
                ActivityManagerNative.getDefault().getTopTask(leftStackId);
             if (rightTask != null) {
                rightComponentInfo = rightTask.baseIntent.getComponent();
             }
             if (leftTask != null) {
                leftComponentInfo = leftTask.baseIntent.getComponent();
             }

        } catch (RemoteException e) {
            Log.e(TAG, "get stackId faile", e);
        }
        //step3: rebuild app tiles
        //step4: remove the top running app icon
        for (AppInfo af : mAppList){
            int viewIndex = mContainerView.indexOfChild(af.appTile);
            if (viewIndex < 0 && ((leftComponentInfo != null
                && (af.packageName).equals(leftComponentInfo.getPackageName()))
                || (rightComponentInfo != null
                && (af.packageName).equals(rightComponentInfo.getPackageName())))) {
                    continue;
        }
        if (viewIndex < 0) {
            mContainerView.addView(af.appTile);
        }
        if ((leftComponentInfo != null && (af.packageName).equals(leftComponentInfo.getPackageName()))
            || (rightComponentInfo != null
            && (af.packageName).equals(rightComponentInfo.getPackageName()))) {
            mContainerView.removeView(af.appTile);
        }
        }
        updateResources();
    ...
    void updateResources() {
        ...
        int span = 2;
        if (SPLITSCREEN) {
            span = r.getInteger(R.integer.quick_settings_user_time_settings_tile_span_splitscreen);
        } else {
            span = r.getInteger(R.integer.quick_settings_user_time_settings_tile_span);
      	}
    ...
    private final BroadcastReceiver mReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (SPLITSCREEN && action.equals(Intent.ACTION_PACKAGE_ADDED)) {
                ...
                try {
                    ...
                    Log.e(TAG, "Couldn't get app info", e);
                }
                if (appIntent == null) {
                        return;
                }
                QuickSettingsTileView userTile = (QuickSettingsTileView)
                    inflater.inflate(R.layout.quick_settings_tile, mContainerView, false);
                userTile.setContent(R.layout.quick_settings_tile_app, inflater);
                TextView appNameView = (TextView) userTile.findViewById(R.id.app_textview);
                ImageView iconView = (ImageView) userTile.findViewById(R.id.app_imageview);

                appIntent.addFlags(Intent.FLAG_ACTIVITY_RUN_IN_RIGHT_WINDOW);
                appNameView.setText(lable);
                iconView.setImageDrawable(appIcon);
                final Intent launchIntent = appIntent;
                final String pgn = packageName;
                userTile.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View v) {
                ...
                mContainerView.addView(userTile);
                AppInfo tileInfo = new AppInfo(userTile, lable, pgn);
                mAppList.add(tileInfo);

            } else if (SPLITSCREEN && action.equals(Intent.ACTION_PACKAGE_REMOVED)) {
                String packageName = intent.getData().getSchemeSpecificPart();
                for (AppInfo af : mAppList) {
                    if (af.packageName.equals(packageName)) {
                        mContainerView.removeView(af.appTile);
                        mAppList.remove(af);
                        break;
                      }
