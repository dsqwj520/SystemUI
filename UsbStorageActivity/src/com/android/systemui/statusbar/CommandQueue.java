...
public class CommandQueue extends IStatusBar.Stub {
    ...
    private static final int MSG_RESET_STATUS_APP_ICON      = 20 << MSG_SHIFT;
    private static final int MSG_UPDATE_SPLIT_WIN_ICON      = 21 << MSG_SHIFT;
    private static final int MSG_SET_SPLIT_BUTTON_VISIBLE   = 22 << MSG_SHIFT;
    private static final int MSG_OPEN_FRONT_CAM             = 23 << MSG_SHIFT;
    ...
    public void resetAppIcon() {
        synchronized (mList) {
            mHandler.removeMessages(MSG_RESET_STATUS_APP_ICON);
            mHandler.obtainMessage(MSG_RESET_STATUS_APP_ICON, 0, 0, null).sendToTarget();
        }
    }

    public void updateSplitCtrlwin() {
        synchronized (mList) {
            mHandler.removeMessages(MSG_UPDATE_SPLIT_WIN_ICON);
            mHandler.obtainMessage(MSG_UPDATE_SPLIT_WIN_ICON, 0, 0, null).sendToTarget();
        }
    }

    public void setSplitButtonVisibility(boolean isVisible) {
        synchronized (mList) {
            mHandler.removeMessages(MSG_SET_SPLIT_BUTTON_VISIBLE);
            mHandler.obtainMessage(MSG_SET_SPLIT_BUTTON_VISIBLE, isVisible ? 1 : 0, 0, null).sendToTarget();
        }
    }

    public void openFrontCam() {
        synchronized (mList) {
            mHandler.removeMessages(MSG_OPEN_FRONT_CAM);
            mHandler.obtainMessage(MSG_OPEN_FRONT_CAM, 0, 0, null).sendToTarget();
        }
    }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (what) {
                ...
                case MSG_SET_WINDOW_STATE:
                    ...
                    break;
                case MSG_RESET_STATUS_APP_ICON:
                    mCallbacks.resetAppIcon();
                    break;
                case MSG_UPDATE_SPLIT_WIN_ICON:
                    mCallbacks.updateSplitCtrlwin();
                    break;
                case MSG_SET_SPLIT_BUTTON_VISIBLE:
                    mCallbacks.setSplitButtonVisibility(msg.arg1 != 0);
                    break;
                case MSG_OPEN_FRONT_CAM:
                    mCallbacks.openFrontCam();
                    break;
            }
        }
    }
